...
class SongTagsDialogFragment : DialogFragment() {
    ...
    /** Eliminable = tiene membresía real: Vídeo sincronizado, Remix / Cover o una personalizada
     *  (systemKey null y no [TagSummary.isAutoAssigned]). Las 3 calculadas nunca lo son, no hay fila
     *  que borrar; una de idioma tampoco -SÍ tiene fila real, pero se gestiona sola desde el editor de
     *  metadatos, no a mano desde aquí (ver [TagsViewModel.isEditable])-. Mismo criterio que
    ...
    private fun isRemovable(tag: TagSummary) =
        (tag.systemKey == null && !tag.isAutoAssigned) ||
